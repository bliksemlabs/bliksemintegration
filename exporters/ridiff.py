...
conn = psycopg2.connect("dbname='ridacc'")
cur = conn.cursor(cursor_factory=psycopg2.extras.DictCursor)

cur.execute("""
SELECT
'@'||to_char(0, 'FM000'),
to_char(date 'yesterday', 'YYYYMMDD'),
to_char(max(validdate), 'YYYYMMDD'),
to_char(1, 'FM0000'),
...
open('output/delivery.dat', 'w').write(delivery)
...
'0000' as timezone,
'0' as obsolute,
to_char(rd_x, 'FM000000') as xcoordinate,
to_char(rd_y, 'FM000000') as ycoordinate,
left(s_pt.name, 29) || repeat(' ', 29 - length(left(s_pt.name, 29))) as stationname
FROM (
        SELECT
...
cur.execute("""
select to_char(stoparearef, 'FM000000') as groupshortname, left(stoparea.name, 29) || repeat(' ', 29 - length(left(stoparea.name, 29))) as groupname,
...
while True:
    ...
    f.write('#' + out['groupshortname'] + ',' + out['groupname'] + '\r\n')
...
to_char(s_pt.id, 'FM0000000') as stationshortname,
...
WHERE totaldrivetime is not null ORDER BY companynumber, serviceidentification, servicenumber, p_pt.pointref;
""");
...
def render(f, trip):
    ...
    f.write( '%' + ','.join([trip[0]['companynumber'], trip[0]['servicenumber'], trip[0]['variant'], '000', '999', trip[0]['servicename']]) + '\r\n' ) # Service Record
    f.write( '-' + ','.join([trip[0]['footnotenumber'], '000', '999']) + '\r\n') # Validity
    f.write( '&' + ','.join([trnsmode[trip[0]['trnsmode'].upper()], '000', '999']) + '\r\n') # Transport mode Record
